<?php 
	if(isset($_GET['id']) && isset($_GET['idnsx'])){
		$idLoaiSP = $_GET['id'];
		$idnsx = $_GET['idnsx'];
		$strQuery = "SELECT sp.ID, sp.TenSanPham, sp.Gia, sp.ChiTiet, sp.Img, nsx.NhaSanXuat FROM `sanpham` sp, `nhasanxuat` nsx WHERE sp.MaNhaSanXuat = nsx.ID AND sp.MaNhaSanXuat=$idnsx AND sp.MaLoai=$idLoaiSP AND sp.DaXoa=0";
		$sql = "$strQuery ORDER BY sp.ID DESC LIMIT 9;";
